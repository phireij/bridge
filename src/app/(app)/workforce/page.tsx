...
export default async function WorkforcePage() {
  ...
  const byDepartment = new Map<string, typeof workforce>();
  for (const w of workforce) {
    const key = w.departmentName ?? "Unassigned";
    byDepartment.set(key, [...(byDepartment.get(key) ?? []), w]);
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow="Your team"
        title="AI Workforce"
        description="Real operational state for CEO, CTO, HyperAgent, and Hermes — bound to their own authenticated accounts and grouped by department, not a seeded roster."
      />
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        />
        <StatCard label="Departments" value={String(byDepartment.size)} hint="tracked" />
      ...
      {workforce.length === 0 ? (
        ...
        Array.from(byDepartment.entries()).map(([department, members]) => (
